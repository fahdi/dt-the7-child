...
function format_phone_number_value_with_link( $value, $post_id, $field ) {
	// Check if the value is not empty and is a phone number
	if ( ! empty( $value ) && is_numeric( $value ) && strlen( $value ) == 10 ) {
		// Format the phone number
		$formatted_value = sprintf( "(%s) %s-%s",
		...
		// Wrap the formatted number in a "tel:" link
		$tel_link = sprintf( '<a href="tel:%s">%s</a>', $value, $formatted_value );

		// Return the linked, formatted phone number
		return $tel_link;
	}
...
// Apply the filter for each phone field, updating the function name if necessary
add_filter( 'acf/format_value/name=phone', 'format_phone_number_value_with_link', 10, 3 );
add_filter( 'acf/format_value/name=phone_2', 'format_phone_number_value_with_link', 10, 3 );
add_filter( 'acf/format_value/name=phone_3', 'format_phone_number_value_with_link', 10, 3 );
add_filter( 'acf/format_value/name=phone_4', 'format_phone_number_value_with_link', 10, 3 );
add_filter( 'acf/format_value/name=phone_5', 'format_phone_number_value_with_link', 10, 3 );
